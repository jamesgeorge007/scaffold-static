...
const bootstrapThreeTemplate = fs.readFileSync(`${templatePath}/bootstrap-3/index.html`, 'utf-8');
const bootstrapFourTemplate = fs.readFileSync(`${templatePath}/bootstrap-4/index.html`, 'utf-8');
const materializeTemplate = fs.readFileSync(`${templatePath}/materialize/index.html`, 'utf-8');
const foundationTemplate = fs.readFileSync(`${templatePath}/foundation/index.html`, 'utf-8');
const semanticUiTemplate = fs.readFileSync(`${templatePath}/semantic-ui/index.html`, 'utf-8');
const bulmaTemplate = fs.readFileSync(`${templatePath}/bulma/index.html`, 'utf-8');
const basicTemplate = fs.readFileSync(`${templatePath}/none/index.html`, 'utf-8');

const serveTemplate = (projectName) => {
    opn(`${projectName}/index.html`);
    console.log(chalk.greenBright(`\n Generated the following files within ${require('path').join(process.cwd(), projectName)}`));
...
exports.scaffoldProject = async (projectName) => {
    ...
    const args = process.argv.slice(3);
        ...
        if (args.length > 1){
        	console.log(chalk.red.bold(' Kindly provide only one argument as the project name!!'));
        	process.exit(1);
        }
